"""OJT 작성 도우미의 로컬 런타임과 데이터 처리 경계.

이 모듈은 네 가지 역할을 한 프로세스에서 조정한다.

* Windows 활성 창과 의미 있는 UI 클릭을 수집한다.
* 민감한 제목·레이블을 DPAPI로 암호화해 SQLite에 저장한다.
* 활동을 업무 후보와 OJT 초안으로 정리하며 로컬 규칙을 항상 폴백으로 둔다.
* 허용된 정적 파일과 ``/api/*`` JSON만 127.0.0.1에 제공한다.

브라우저 화면과 ``mini_app.py``가 모두 이 API를 사용하므로 응답 스키마를 바꿀 때는 두
클라이언트를 함께 확인해야 한다. 데이터·보안 경계는 ``docs/FILE_GUIDE.md``를 참고한다.
"""
